    ...
    for string in strings:
        ...
        if "_" in string:
            ...
            for part in parts[1:]:
                split_result.append(
                    "_" + part
                )  # Append the remaining parts with an underscore
        else:
...
def extract_last_two_turns(dialogue):
    ...
    return remaining_dialogue, [
        last_two_turns[0].split("\n", 1)[0],
        last_two_turns[1].split("\n", 1)[0],
    ]
...
def combine(
    ...
    second_sentence="Question",
):

    str_output = (
        previous_reply.replace(".", ":")
        + " "
        + last_two_turns[0]
        + " "
        + last_two_turns[1]
...
def generate_dialogue(topic, topic_type, context, list_of_intents, first_question):
    ...
    message = (
        general_instruction_first_turn
    ...
    for i in range(1, len(list_of_intents)):
        ...
        turn = conversational_turns[
            conv_turns[i]
        ]  ##IF _ we keep the turn one more round; but using different intents
        ...
        if "_" in list_of_intents[i]:
            instruction_to_message = combine_instructionv2(
                list_of_intents[i],
                intents_dict,
                intents_dict_secondkey=turn + " instruction",
            )
...
with jsonlines.open(path_generated_dialogues, mode="w") as writer:
    with jsonlines.open(path_entity_cards) as reader:
        for obj in tqdm.tqdm(
            reader, desc="itterating on entity cards and sequence of intents..."
        ):
            # print(obj)
            try:
                cnt += 1
                utterances_with_intents, combined_instructions = generate_dialogue(
                    obj["entity_name"].strip(),
                    obj["entity_type"].strip(),
                    obj["entity_description"].strip(),
                    obj["sequence_of_intents"],
                    obj["conversation_starter"].strip(),
                )
                utterances_with_intents["entity_card_obj"] = obj
                all_combined_instructions = (
                    all_combined_instructions | combined_instructions
                )
